...
export function formatServiceStatus(
  ...
  if (!status.supported) {
    return "Rearvy Coding Agent service\n  Status: unavailable on this machine\n  Supported on: Linux with systemd, macOS with launchd";
  }
  if (!status.installed) {
    return "Rearvy Coding Agent service\n  Status: not installed\n  Next: Run `t3 service install`.";
  }
  return [
    "Rearvy Coding Agent service",
...
const serviceInstallCommand = Command.make("install", projectLocationFlags).pipe(
  Command.withDescription("Install Rearvy Coding Agent as a background service for this user."),
  Command.withHandler((flags) =>
    runServiceCommand(
      ...
      Effect.gen(function* () {
        ...
        if (!result.changed) {
          yield* Console.log(
            `Rearvy Coding Agent service is already installed with t3@${packageJson.version}.`,
          );
          return;
        }
        yield* Console.log(
          `${result.previouslyInstalled ? "Updated" : "Installed"} Rearvy Coding Agent service with t3@${packageJson.version}.\nLogs: ${result.plan.logPath}`,
        );
      }),
    ),
  ),
  ...
  Command.withHandler((flags) =>
    runServiceCommand(
      ...
      Effect.gen(function* () {
        ...
        if (!result.changed) {
          yield* Console.log(
            `Rearvy Coding Agent service is already using t3@${packageJson.version}.`,
          );
          return;
        }
        yield* Console.log(
...
const serviceUninstallCommand = Command.make("uninstall", projectLocationFlags).pipe(
  Command.withDescription("Stop and remove the Rearvy Coding Agent background service."),
  Command.withHandler((flags) =>
    runServiceCommand(
      flags,
      Effect.gen(function* () {
        ...
        yield* Console.log(
          removed
            ? "Removed the Rearvy Coding Agent service."
            : "Rearvy Coding Agent service is not installed.",
        );
      }),
    ),
  ),
...
const serviceStatusCommand = Command.make("status", projectLocationFlags).pipe(
  Command.withDescription("Show whether the Rearvy Coding Agent background service is installed."),
  Command.withHandler((flags) =>
    runServiceCommand(
      flags,
      Effect.gen(function* () {
...
export const offerServiceDuringOnboarding = Effect.gen(function* () {
  ...
  if (installed && current) {
    yield* Console.log(
      "Rearvy Coding Agent is already set up to run in the background on this machine.",
    );
    return true;
  }
  ...
  const wanted = yield* Prompt.run(
    Prompt.confirm({
      message: installed
        ? "The installed Rearvy Coding Agent service needs an update or repair. Update it now?"
        : platform === "darwin"
          ? "Run Rearvy Coding Agent in the background whenever you log in to this Mac? " +
            "It stays reachable through T3 Connect while you are logged in."
          : "Run Rearvy Coding Agent in the background whenever this machine boots? " +
...
export const serviceCommand = Command.make("service").pipe(
  Command.withDescription("Manage the Rearvy Coding Agent background service."),
